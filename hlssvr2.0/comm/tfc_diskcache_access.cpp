...
//	return >0, on set fail, no enough position
//
int CacheAccess::set(const char* key, const char* data, unsigned data_len, int expiretime, ENodeFlag flag)
{
	//	find
	TMBHashKey hash_key;
	memcpy(hash_key._.md5_, key, TMBHashKey::C_DATA_LEN);
...
//	return <0 on error
//
int CacheAccess::oldest_key(char* key, unsigned& data_len, bool& dirty_flag, int& time_stamp, int& expiretime)
{
	THashNode* node = _cache.get_add_list_tail();
	if (node == NULL)
		return 1;
...
int CacheAccess::open(char* mem, unsigned mem_size, const std::string& cache_file, bool inited
...
int CacheAccess::StartUp()
{
	...
	while(0<(len = _binlog.ReadRecordFromBinLog(szBuff, MAX_BINLOG_ITEM_LEN)))
	{
		...
		if (op == op_set)
		{
			...
			memcpy(&expiretime, szBuff+sizeof(int)+TMBHashKey::C_DATA_LEN+sizeof(int), sizeof(int));
...
void  CacheAccess::get_chunk_useinfo(unsigned& uiChunkTotal, unsigned& uiChunkFree, unsigned& uiChunkSize)
{
	return _cache.get_chunk_useinfo(uiChunkTotal, uiChunkFree, uiChunkSize);
}
