...
THashNode* CHashMap::find_node_purely(TMBHashKey &key)
{
	int bucket_id = get_bucket_id(key);
	BC_MEM_HANDLER node_hdr = hash_map_->bucket[bucket_id];
	while(node_hdr != INVALID_BC_MEM_HANDLER)
...
THashNode* CHashMap::insert_node_metadata(TMBHashKey &key, void* new_data, int new_len, int expiretime)
{
	THashNode* node = free_list_remove();
	if (node == NULL) 
	{
...
THashNode* CHashMap::update_node_metadata(THashNode* node, void* new_data, int new_len, int expiretime)
{
	int old_chunk_num = allocator_.get_chunk_num(node->chunk_len_);
	int new_chunk_num = allocator_.get_chunk_num(new_len);
